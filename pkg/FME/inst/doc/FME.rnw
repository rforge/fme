...
%\VignetteIndexEntry{FME - sensitivity, calibration, identifiability, monte carlo analysis for models consisting of differential equations}
...
%% ThPe Suggestion: more meaningful figure main,
%% e.g. state variables and not "Sensitivity rB"
...
%% ThPe: Is latin hypercube a distribution?? I think it's a sampling algorithm
...
%% ThPe --> KS: Please give an explanation of the summary output,
%% esp. L1 and L2
...
Function \code{modCRL} runs a Monte Carlo simulation, outputting 0-D variables,
(in contrast to sensRange which outputs 1-D variables).

% 1) ThPe -> KS: Please explain shortly the difference between 0-D and
% 1-D variables.

% 2) ThPe -> KS: The example *figure* is not conclusive showing the
% advantage of Monte Carlo for me. I use MC if I want to see
% propagation of errors with a specific *distribution*. i.e. to derive
% the output distribution from an input distribution; mostly (but not
% exclusively) for the multivariate case
...
%% ThPe --> KS: identifiable *data* set or *parameter* set??
...
% ThPe -> KS: thanks for including a simecol object
