...
\title{\proglang{R}-package \fme: utilities for the application of
  models consisting of differential equations}

\Plaintitle{FME}

\Keywords{simulation models, differential equations, fitting,
  sensitivity, monte carlo, identifiability, \proglang{R}}

\Plainkeywords{simulation models, differential equations, fitting,
...
\emph{E}nvironment for \emph{M}athematically \emph{M}odelling the
\emph{E}nvironment.

\code{FEMME} was written in FORTRAN. \fme is -obviously- written in
\R.
...
\citep{rootSolve} which estimate steady-state conditions.
...
this document a -- very quick -- survey of the functionality are
...
substrate (\code{SUB}), and in a closed vessel.
...
the time sequence at which output is wanted.

Within this function, \code{derivs} is defined, which is the
\emph{derivative} function, called at each time step by the solver. It
takes as input the current time (\code{t}), the current values of the
state variables (\code{state}) and the parameters (\code{pars}). It
returns the rate of change of the state variables, packed as a list.

Also within function \code{solveBact}, the state variables are given
an initial condition (\code{state}) and the model solved by
integration, using function \code{ode} from package
\code{deSolve}. The results of the integration are returned, packed as
a data.frame.
...
The model is solved by calling \code{solveBact} with the default
parameters:
...
First the sensitivity parameters are defined and a distribution
...
each time step (\code{x}).

It is relatively simple to plot the ranges, either as min+-std or
using quantiles.
...
parameters. Here we use all 3 parameters, and select a latin hypercube
distribution.
...
in \fme are based on \citep{Brun} which should be consulted for
...
Function \code{modCRL} runs a monte carlo, outputting 0-D variables,
(in contrast to sensRange which outputs 1-D variables).
...
for the crosstab format. Note, that in the latter case it is not
...
modelfitting, running a markov chain monte carlo.

Assume the following model output (in a matrix or data.frame called
\code{Mod}
...
First it is instructive to establish which parameters are identifiable
...
\item the model variance \code{modVarianc} is used as the initial
  model error variance (\code{var0}).  In each MCMC step, 1/model
  variance is drawn from a gamma function with parameters
  shape=0.5*N*(1+pvar0), and the rate=0.5*(pvar0*N*var0+SSnew)) and
  where SS is the current sum of squared residals, N is the number of
  data points and \code{pVar0} is a weighing parameter, input to
  \code{modMCMC}.
...
\begin{itemize}
\item BODO2\_FME.R, a 1-D model of oxygen dynamics in a river. This
  model consists of two coupled partial differential equations, which
  are solved to steady-state.
\item ccl4model\_FME.R. Here the functions are applied to "ccl4model",
  one of the models included in package \ds. This is a model that has
  been written in FORTRAN.
\item Omexdia\_FME.R. Here the functions are applied to a model
...
\item O2profile\_FME.R. This contains a simple model of oxygen,
...
This vignette is mainly a Sweave \citep{Leisch02} translation of part
of the \fme help files and examples.
