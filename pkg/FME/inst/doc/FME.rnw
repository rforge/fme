...
\item A function to perform parameter identifiability analysis
\item A function to estimate the model cost function, useful for fitting a model to data.
\item Functions to fit a model to data.
\item A function to run a markov chain monte carlo (MCMC).
...
metropolis (AM) algorithm (\citep{Haario05})
