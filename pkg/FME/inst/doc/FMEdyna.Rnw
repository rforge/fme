...
dynamic simulation model, solved with integration routines from
package \ds.
...
%% ThPe --> KS: I formatted the following formula as \code, but it
%% would be also possible (and preferred?) to use mathematical
%% notation, however, this would require to change also the symbols
%% (e.g. pvar0) and functions (e.g. var) to their mathematical
%% counterparts. If you agree with the technical style here, remove
%% this comment. BTW: I suggest to use more technical style in
%% vignettes and mathematics in publications.


\begin{itemize}

\item The model variance \code{modVariance} is used as the initial
  model error variance (\code{var0}).  In each MCMC step, \code{1/model}
  variance is drawn from a gamma function with parameters
  \code{shape = 0.5*N * (1 + pvar0)}, and the \code{rate = 0.5 * (pvar0*N*var0 + SSnew))} and
