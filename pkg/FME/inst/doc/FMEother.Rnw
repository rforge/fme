...
Finally, we run an MCMC analysis. The -scaled- parameter covariances returned
...
The predictive posterior distribution of the model is easily estimated by running
...
@
\end{center}
\caption{Predictive envelopes of the model - see text for \R-code}
\label{fig:Monsum2}
\end{figure}
