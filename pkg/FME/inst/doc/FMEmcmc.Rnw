...
Here some functions from \R-package \fme are tested. As it is a test of the
function implementation, this does not mean that the results obtained are meaningful.
...
@
\end{center}
\caption{Pairs plot of the Adaptive Metropolis MCMC of the chemical model - see text for \R-code}
\label{fig:ABMCMC3}
\end{figure}

\section{finally}
This vignette is a Sweave \citep{Leisch02} translation of part of the \fme examples.
