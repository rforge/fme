...
while for 1/the error variance, a Gamma distribution is used as a prior:

\[
p(\sigma^{-2}) \sim \Gamma(\frac{n_0}{2},\frac{n_0}{2}S_0^2)
\]

The posterior for the parameters will be estimated as:
  \[
  p(\theta | y,\sigma^2)\propto exp(-0.5*(\frac{SS(\theta)}{\sigma^2}
  +SS_{pri}(\theta))
  \]
  and where $\sigma^2$ is the error variance, SS is the sum of squares
  function \[SS(\theta)=\sum(y_i-f(\theta)_i)^2\] and
  \[SS_{pri}(\theta)=\sum_i{(\frac{\theta_i-v_i}{\mu_i})^2}\]
...
\code{what=NULL} does that (figure \ref{fig:Monmcmc2}).
<<label=Monmcmc2, include=FALSE>>=
plot(MCMC2,Full=TRUE,what=NULL)
@
\setkeys{Gin}{width=0.8\textwidth}
\begin{figure}
\begin{center}
